...
class ProductFilter(FilterSet):
	...
	min_price = NumberFilter(name='price', lookup_expr='gte', distinct=True) # (some_price__gte=somequery)
	max_price = NumberFilter(name='price', lookup_expr='lte', distinct=True)
	class Meta:
		model = Product
		fields = [
			'min_price',
